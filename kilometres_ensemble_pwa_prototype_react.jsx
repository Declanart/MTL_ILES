...
/**
 * Kilomètres Ensemble — simple group distance tracker (PWA-friendly)
 * ---------------------------------------------------------------
 * ✅ No backend needed for demo; uses localStorage
 * ✅ Works offline; can be added to Home Screen (PWA once hosted)
 * ✅ Clean UI with Tailwind; responsive on mobile
 *
 * How it works
 * - First screen: choose group and your display name
 * - Log your km per day (multiple entries allowed)
 * - Leaderboard sums per person; Progress shows group total vs milestones
 * - Admin tab lets you edit milestones & reset data (unguarded; demo only)
 *
 * Next step to make it multi-device & real-time: plug Firebase (Auth + Firestore)
 * - Replace storage helpers with Firestore CRUD
 * - Keep the same data shape below
 */
...
const DEFAULT_GROUP = {
  ...
  members: {}, // { [username]: { entries: [{ id, dateISO, km, note? }] } }
};
...
export default function App() {
  ...
  // Derived: flattened entries & totals
  ...
  function createGroup(name) {
    if (!name) return;
    setState(prev => {
      if (prev.groups[name]) return prev;
      const g = { name, milestones: DEFAULT_MILESTONES, members: {} };
      return { ...prev, groups: { ...prev.groups, [name]: g } };
    });
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-b from-neutral-50 to-neutral-100 text-neutral-900">
      <div className="max-w-xl mx-auto p-4 pb-24">
        ...
        {activeTab === "admin" && (
          <AdminTab milestones={group.milestones} updateMilestones={updateMilestones} resetGroup={resetGroup} />
        )}

        <div className="mt-8 text-center text-xs text-neutral-400">
          Astuce: installe cette app sur ton téléphone via « Ajouter à l'écran d'accueil » après l'avoir hébergée.
        </div>
      </div>
    </div>
  );
    ...
    <div className="space-y-4">
      <Card>
        ...
        <div className="mt-3 flex gap-2">
          ...
          <Button onClick={() => { setKm(3); setDateISO(todayISO()); setNote(""); }}>Réinitialiser</Button>
        </div>
      </Card>

      <Card>
        <div className="flex items-center gap-2 mb-2"><Users className="w-4 h-4"/><span className="font-medium">Historique des entrées</span></div>
  ...
  return (
    <div className="space-y-4">
      <Card>
        ...
        <div className="mt-2"><ProgressBar value={groupTotal} max={max} /></div>
        {nextMilestone && (
...
function AdminTab({ milestones, updateMilestones, resetGroup }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <Card>
        <div className="font-medium mb-1">Passer au temps réel (plus tard)</div>
        <p className="text-sm text-neutral-600">Pour une version multi-appareils, brancher Firebase (Auth + Firestore) et remplacer localStorage par des collections: <code>groups</code>, <code>users</code>, <code>entries</code>. Le reste de l'UI peut rester identique.</p>
      </Card>
    </div>
  );
}
